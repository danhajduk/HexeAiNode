...
DEFAULT_BUDGET_POLICY_CURRENT_PATH = "/api/system/nodes/budgets/policy/current"
...
def _build_budget_policy_url(*, core_api_endpoint: str, budget_policy_path: str, node_id: str) -> str:
    base = _require_non_empty_string(core_api_endpoint, "core_api_endpoint")
    path = _require_non_empty_string(budget_policy_path, "budget_policy_path")
...
class HttpxBudgetPolicyAdapter:
...
class BudgetPolicyClient:
    def __init__(self, *, logger, http_adapter=None) -> None:
        self._logger = logger
        self._http_adapter = http_adapter or HttpxBudgetPolicyAdapter()

    async def fetch_current_policy(
        self,
        *,
        core_api_endpoint: str,
        trust_token: str,
        node_id: str,
        budget_policy_path: str = DEFAULT_BUDGET_POLICY_CURRENT_PATH,
    ) -> BudgetPolicyFetchResult:
        url = _build_budget_policy_url(
        ...
        if hasattr(self._logger, "info"):
            self._logger.info("[budget-policy-request] %s", {"url": url, "node_id": node_id})
        status_code, payload = await self._http_adapter.get_json(url, headers)
        return _classify_budget_policy_response(status_code=status_code, payload=payload)


def _classify_budget_policy_response(*, status_code: int, payload: dict) -> BudgetPolicyFetchResult:
    if status_code >= 500 or status_code in {408, 425, 429}:
        return BudgetPolicyFetchResult(
    ...
    return BudgetPolicyFetchResult(status="synced", payload=payload, retryable=False, error=None)
